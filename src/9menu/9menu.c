...
memory(char *s)
{
	...
	//exit(1); comment line out as calls to memory always needlessly end in an exit
}
...
main(int argc, char **argv)
{
	...
	if (fgcname == NULL
		...
		black = /*invert to overcome NX11 color stub return Black*/WhitePixel(dpy, screen);
	else
		black = color.pixel;
	...
	if (bgcname == NULL
	    ...
	    || XAllocColor(dpy, defcmap, &color) == 0)
		white = /*invert to overcome NX11 color stub return White*/BlackPixel(dpy, screen);
	else
		white = color.pixel;
	...
	gv.foreground = black;//^white; dont OR to overcome NX11 color stub return
	gv.background = white;
	gv.font = font->fid;
	gv.function = GXxor;
	gv.line_width = 0;
